...
-- Duas sem inscrição nenhuma: os testes de venda precisam de alguém que ainda
-- não tenha o ingresso "um por pessoa", e reaproveitar as de cima faria o
-- resultado depender da ordem em que os testes rodam.
insert into pessoas (id, cpf, nome) values
  ('aaaa0000-0000-0000-0000-000000000003', '11144477735', 'Joana de Teste'),
  ('aaaa0000-0000-0000-0000-000000000004', '12345678909', 'Carlos de Teste');
...
-- ⚠️ AS SEQUÊNCIAS PRECISAM ANDAR. As linhas acima entram com `id` explícito,
-- e `generated by default as identity` NÃO avança a sequência quando o valor
-- vem escrito: a próxima inserção sem id tentaria o 1, e o que o teste gravar
-- some no meio da semente em vez de aparecer depois dela. Foi assim que a
-- primeira versão desta suíte "vendeu" quatro vezes sem achar uma linha.
select setval(pg_get_serial_sequence('eventos.eventos', 'id'),
               (select max(id) from eventos.eventos));
select setval(pg_get_serial_sequence('eventos.ingresso_tipos', 'id'),
               (select max(id) from eventos.ingresso_tipos));
select setval(pg_get_serial_sequence('eventos.ingresso_campos', 'id'),
               (select max(id) from eventos.ingresso_campos));
select setval(pg_get_serial_sequence('eventos.combos', 'id'),
               (select max(id) from eventos.combos));
select setval(pg_get_serial_sequence('eventos.combo_itens', 'id'),
               (select max(id) from eventos.combo_itens));
select setval(pg_get_serial_sequence('eventos.cupons', 'id'),
               (select max(id) from eventos.cupons));
select setval(pg_get_serial_sequence('eventos.inscricoes', 'id'),
               (select max(id) from eventos.inscricoes));
select setval(pg_get_serial_sequence('eventos.pedidos', 'id'),
               (select max(id) from eventos.pedidos));
