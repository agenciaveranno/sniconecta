...
export default async function PainelPage({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  // `pronto` antes da capacidade: tela que não existe não vira atalho, por
  // mais permissão que a pessoa tenha.
  const grupos = MODULOS.map((m) => ({
    ...m,
    itens: m.itens.filter((i) => i.pronto && eu?.pode(i.capacidade)),
  })).filter((m) => m.itens.length > 0);
  const total = grupos.reduce((n, m) => n + m.itens.length, 0);
  ...
  return (
    <Painel titulo="Início">
      ...
      {total === 0 ? (
        ...
        // Agrupado por módulo, e não numa grade só: "Configurações" existe em
        // mais de um módulo, e dois cartões com o mesmo nome lado a lado não
        // dizem qual é qual.
        grupos.map((m) => (
          <section key={m.chave} style={{ marginBottom: 28 }}>
            <TituloSecao>{m.rotulo}</TituloSecao>
